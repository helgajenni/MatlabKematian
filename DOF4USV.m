...
ILOS.Delta    = 2.5;    % Jarak pandang aman agar tidak panik di awal
ILOS.gamma    = 0.5;   
ILOS.beta_hat = 0;
ILOS.kappa    = 0.25;    % Turunkan drastis atau jadikan 0 agar tidak terjadi windup
WP_RADIUS  = 1.5;   
WP_PAUSE   = 3.5;   
GOAL_TOL   = 1.0;   % Kembalikan ke 1.0 agar dia bisa mendeteksi sudah sampai   

% Kembalikan ke tuning proporsional yang stabil
PD.Kp = 0.8;            
PD.Kd = 1.0;
U0_saved = USV.U0; 
...
for k = 1:N
  ...
  %% 3. ADVANCE WAYPOINT INDEX & TANGENT LINTASAN YANG STABIL
    ...
    cte = ye;
    if abs(ye) > 5.0
        ILOS.beta_hat = 0;  % FIX: threshold diperbesar agar tidak reset terlalu sering
    end
    
    % --- ADAPTIVE DELTA (Mengecil saat dekat WP tapi dibatasi min 2.5m) ---
    dist_wp_junction = norm([xk - waypoint(1), yk - waypoint(2)]);
    wp_zone = 5.5; 
    Delta_eff = ILOS.Delta; 
    if dist_wp_junction < wp_zone
        Delta_eff = max(ILOS.Delta * 0.7, ILOS.Delta * (dist_wp_junction / wp_zone));  % FIX: minimum 70% Delta
    end
    
    ILOS.beta_hat = ILOS.beta_hat + dt * ILOS.kappa * ...
        (ye / sqrt(Delta_eff^2 + (ye + ILOS.gamma*ILOS.beta_hat)^2));
    beta_max = 1.5;  % FIX: clamp agar tidak tumbuh tanpa batas
    ILOS.beta_hat = max(-beta_max, min(beta_max, ILOS.beta_hat));
        
    psi_d = wrapToPi(alpha - atan2(ye + ILOS.gamma*ILOS.beta_hat, Delta_eff));
    
   %% 5. PD CONTROLLER (RUDDER)
    psi_e = wrapToPi(psi_d - psi);
    
    % FIX: d(psi_e)/dt = d(psi_d)/dt - r, harus ikutkan laju perubahan heading desired
    psi_d_dot = wrapToPi(psi_d - log.psi_d(max(k-1,1))) / dt;
    dpsi_e = psi_d_dot - r;
   ...
   %% 6. KECEPATAN ADAPTIF (MELAMBAT DI TIKUNGAN)
    futureCurvature = abs(pathCurv(min(wpIdx + 10, nWP)));
    
    if dist_goal < GOAL_TOL
        USV.U0 = 0.0; 
    elseif dist_goal < 5.0
        USV.U0 = 0.5; 
    % UBAH BARIS DI BAWAH INI: Toleransi heading dan kelengkungan diperbesar
    elseif abs(psi_e) > deg2rad(40) || futureCurvature > 0.2  % FIX: melambat lebih awal
        USV.U0 = 1.1; 
    else
        USV.U0 = 1.5; 
    end
