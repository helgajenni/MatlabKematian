...
seed_value = 125; % Gunakan seed ini, terbukti menghasilkan jalur RRT* yang rapi
rng(seed_value);
...
% Rintangan digeser ke tengah agar saling menutupi jalur lurus!
obstacles = [
    19.5, 12, 1.5;  % O1 (Kanan) -> Memaksa kapal jauh ke KIRI
    15.5, 22, 1.5;  % O2 (Kiri)  -> Memaksa kapal jauh ke KANAN
    19.5, 32, 1.5;  % O3 (Kanan) -> Memaksa kapal jauh ke KIRI
    15.5, 42, 1.5;  % O4 (Kiri)  -> Memaksa kapal jauh ke KANAN
];
...
% --- PARAMETER IDENTIFIKASI HASIL TA KATING (3-DOF) ---
USV.Xu  = -0.3228;
USV.Xuu = -2.7163;
USV.Yv  = -0.4046;    
USV.Yvv = -20.6620;
...
ILOS.Delta    = 1.8;    % DIPERBESAR: Jarak pandang jauh agar manuver belok lebih antisipatif dan tidak kaget
ILOS.gamma    = 0.0;   
ILOS.beta_hat = 0;
ILOS.kappa    = 0.0;    % DINONAKTIFKAN: Hindari windup (penumpukan error) saat tikungan tajam

WP_RADIUS  = 2.0;   % Diperbesar agar kapal tidak memaksakan diri mencapai titik presisi di celah sempit
WP_PAUSE   = 0.0;   
GOAL_TOL   = 2.0;   

% Tuning PD agar lebih rileks dan tidak osilasi
PD.Kp = 1.7;        % DITURUNKAN: Agar kemudi tidak mudah mentok (saturasi)
PD.Kd = 1.5;        % DINAIKKAN: Sebagai "rem" getaran agar kemudi lebih stabil
...
for k = 1:N
    ...
    if ~wpReached && dist_wp < WP_RADIUS
        ...
        ILOS.beta_hat = 0; 
        fprintf('  >>> MENGHAMPIRI WAYPOINT t=%.1fs \n', t);
    end
    pausing = false; 
    
  %% 3. ADVANCE WAYPOINT INDEX & TANGENT LINTASAN YANG STABIL
    ...
    if newIdx >= wpIdx
        wpIdx = newIdx;  % FIX: hanya maju, tidak boleh mundur
    end
    log.pathSeg(k) = 1 + (wpIdx > seg1Len);   
    
    % FIX: Jarak pandang tangent diperjauh ke 25 titik (~2.5 meter) agar kemudi tidak mudah bergetar
    lookAhead = max(3, round(8 * (USV.U0 / 1.5)));  % FIX: adaptif, lebih pendek
    % FIX: Gunakan 12 titik, ini paling pas untuk keseimbangan
    idxNext = min(wpIdx + 17, nWP); 
    ...
    % FIX UTAMA: Batasi sudut pendekatan (approach angle) maksimal 45 derajat
    % Ini mencegah kapal berbelok terlalu patah yang menyebabkan looping!
    approach_angle = atan2(ye, Delta_eff);
    limit_angle = deg2rad(60); 
    approach_angle = max(-limit_angle, min(limit_angle, approach_angle));
        
    psi_d = wrapToPi(alpha - approach_angle);
    
   %% 5. PD CONTROLLER (RUDDER)
    psi_e = wrapToPi(psi_d - psi);
    
    % FIX UTAMA: Hapus psi_d_dot. Perubahan target heading akibat diskritisasi titik 
    % akan membuat 'derivative kick' (kemudi kaget). Cukup redam yaw rate (r) saja.
    dpsi_e = 0 - r; 
    
    % Hitung command rudder
   ...
   %% 6. KECEPATAN ADAPTIF (MELAMBAT DI TIKUNGAN & SAAT MELENCENG)
    futureCurvature = abs(pathCurv(min(wpIdx + 10, nWP)));
    
    if dist_goal < GOAL_TOL
        USV.U0 = 0.0; 
    elseif dist_goal < 4.0
        USV.U0 = 0.5; 
    % FIX UTAMA: Tambahkan abs(cte) > 1.5 sebagai pemicu rem otomatis!
    elseif abs(psi_e) > deg2rad(25) || futureCurvature > 0.12 || abs(cte) > 0.8
        USV.U0 = 0.6; % Drop kecepatan untuk menstabilkan kapal
    else
        USV.U0 = 1.5; 
    end
...
for i=1:size(obstacles,1)
    ...
    text(ax2,xc,yc,sprintf('O%d',i),'HorizontalAlignment','center','FontSize',9,'FontWeight','bold');
end
...
%% =================== ANALISIS ERROR CTE =========================
% Ambil data CTE dari awal sampai simulasi berhenti (Np)
cte_actual = log.cte(1:Np);

% 1. MAE (Mean Absolute Error) - Rata-rata deviasi / jarak dari garis
MAE_CTE = mean(abs(cte_actual));

% 2. RMSE (Root Mean Square Error) - Sensitif terhadap lonjakan error besar
RMSE_CTE = sqrt(mean(cte_actual.^2));

% 3. Maximum CTE - Jarak melenceng paling jauh
...
function ds = shipEOM(s, delta, USV)
    ...
    m = USV.m; mx = USV.mx; my = USV.my; Iz = USV.Iz; Jz = USV.Jz; Ix = USV.Ix; Jx = USV.Jx;
    if isfield(USV, 'T0_active'), nom_T = USV.T0_active; else, nom_T = USV.T0; end
    
    T = nom_T + 5.0 * (USV.U0 - u); T = max(0, T); 
    
    Ur = max(sqrt(u^2 + v^2), 0.05);
    FY_r = USV.Ky_r * Ur^2 * sin(delta); FN_r = USV.Kn_r * Ur^2 * sin(delta);
    
    X = T + USV.Xuu * u * abs(u) + (m + my) * v * r;
    Y = USV.Yv*v + USV.Yr*r + USV.Yvv*v*abs(v) + FY_r - (m+mx)*u*r;
    N = USV.Nv*v + USV.Nr*r + USV.Nrr*r*abs(r) + FN_r;
    K = USV.Kv_roll*v + USV.Kr_roll*r + USV.Kp_roll*p + USV.Kpp*p*abs(p) - m*9.81*USV.GMT*sin(phi);
      
    u_dot = X / (m + mx); v_dot = Y / (m + my);
    r_dot = N / (Iz + Jz); p_dot = K / (Ix + Jx);
    x_dot = u * cos(psi) - v * sin(psi); y_dot = u * sin(psi) + v * cos(psi);
...
function runAnimation(tab,state,fullPath,rawAll,sP1,sP2,obstacles,USV,log,N_end,dt,xMax,yMax,margin,goal,startPt,wayptPt,ILOS,c1,c2,theta_c,U0_saved)
    ...
    title(axA,'USV  RRT* + G2CBS + ILOS + PD','Color',[.95 .95 1],'FontSize',12,'FontWeight','bold');
